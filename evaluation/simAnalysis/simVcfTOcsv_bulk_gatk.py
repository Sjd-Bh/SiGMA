...
if __name__ == "__main__":
    ...
    for single_cell_file in single_cell_files:
        ...
        # Extract the main folder (e.g., '200' or '400') and the simulation name (e.g., 'sim1')
        main_folder = os.path.basename(os.path.dirname(os.path.dirname(single_cell_file)))  # e.g., '200'
        sim_name = os.path.basename(os.path.dirname(single_cell_file))  # e.g., 'sim1'
        modified_file_name = f"{main_folder}_{sim_name}"
    
        # Rename single cell data columns with the desired pattern
        single_cell_data.columns = [
            f"{col}_{modified_file_name}" if col != 'POS' else col
            for col in single_cell_data.columns
        ]
        ...
        for col in single_cell_data.columns[1:]:
            if col.startswith(('VAF_', 'DP_')) and col in merged_data.columns:
