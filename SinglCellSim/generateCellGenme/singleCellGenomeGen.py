...
def generate_single_cell_genome_from_fasta(reference_genome, genome_name="Single_Cell_Genome", num_snps=10, num_snvs=5, num_cnvs=3):
    """
    ...
    def introduce_snps(seq, num_snps):        
        ...
        for pos in snp_positions:
            ...
            snps.append({'position': pos, 'type': f'{old_nucleotide}->{new_nucleotide}', 'new_nucleotide': new_nucleotide})            
    ...
    def introduce_snvs(sequence, num_snvs):
        snvs = []
        ...
        for pos in snv_positions:
            ...
            snvs.append({'position': pos, 'type': f'{old_nucleotide}->{new_nucleotide}', 'new_nucleotide': new_nucleotide})
    ...
    def introduce_cnvs(sequence, num_cnvs):
        ...
        for _ in range(num_cnvs):
            if len(sequence) >= 2:
                ...
                cnvs.append({'start': cnv_start, 'end': cnv_start + cnv_length})
...
def save_to_vcf(changes, filename, reference_genome):
    with open(filename, "w") as file:
        ...
        for change in changes:
            file.write(f"CHROM\t{change['position']}\t.\t{reference_genome[change['position']]}\t{change['new_nucleotide']}\t.\tPASS\t.\n")
