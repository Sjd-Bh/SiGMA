...
# ---------------------------------------------------------
# 1. Chromosome name based on genome length if not provided (e.g. 1000000 -> 1000kb)
if [ -z "$CHROM" ]; then
    CHROM="$((${GENOME_LENGTH} / 1000))kb"
fi

# 3. Random node if not provided
if [ -z "$NODE" ]; then
    # Generate a random node number (e.g., between 0 and N_CELLS-1)
    NODE=$((RANDOM % N_CELLS))
fi
...
case "$COMMAND" in
  ...
  coalescent)
    ...
    python SinglCellSim/Coal/coalescentSim.py \
  ...
  scDNAseq)
    if [ -z "$REF" ]; then
      echo "Error: --ref is required for scDNAseq"
      exit 1
    fi
    echo "=== Running Single Cell Sequencing Simulation ==="
    
    # 2. SNPs
    python3 SinglCellSim/generateCellGenme/simSNPs.py \
      -o "$SC_OUT/snps" \
      -R "$REF" \
      --rate 0.001
      
    python3 SinglCellSim/generateCellGenme/introduceSNPs.py \
      --ref "$REF" \
    ...
    python SinglCellSim/Coal/treeMutations.py \
    ...
    python SinglCellSim/generateCellGenme/applySNVsCNVs_toCellGenome.py \
    ...
    python SinglCellSim/AmpSim/amp/runAmp_initiation.py \
      --num_simulations "$NUM_SIMS" \
      --config_file SinglCellSim/configs/config_files/MDAsim.ini \
      --output_base "$SC_OUT/amp/MDA/" \
      --patSeq_file "$SC_OUT/cell_genome/8_paternal.fasta" \
      --matSeq_file "$SC_OUT/cell_genome/8_maternal.fasta" \
      --cnv_bed_file "$SC_OUT/mutations/cnv.bed" \
      --num_cores "$CORES"
    
    python SinglCellSim/seqSim/scDNAseqSim.py \
      --ref "$REF" \
    ...
    bash real_data/finding_mean_depth.sh "$SC_OUT/amp/MDA/MDA_bam.txt" -o "$SC_OUT/bam/"
    
    # 5. Determine Target Depth (Min depth extraction)
    ...
    bash real_data/downsam_BQ.sh \
      --input "$SC_OUT/amp/PTA/PTA_bam.txt" \
      --picard /home/bahonar/picard.jar \
      --ref "$REF" \
    ...
    bash real_data/subset_sort_index.sh \
  ...
  bulk)
    ...
    # 7. Bulk Simulation
    python BulkSim/kindred_Normal_bulk_sim.py \
    ...
    python BulkSim/bulk_kindred_normal_sim_to_bam.py \
      --ref "$REF" \
    ...
    # Use dynamically allocated cores for parallel processing
    parallel -j "$CORES" python BulkSim/bulk_kindred_normal_sim_to_bam.py \
      --ref "$REF" \
